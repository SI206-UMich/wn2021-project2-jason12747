...
def get_titles_from_search_results(filename):
    ...
    titles = soup.find_all('tr', itemtype = "http://schema.org/Book")
    
    books = []

    for title in titles:
        bookTitle = title.find(class_="bookTitle")
        authorName = title.find(class_="authorName")
        books.append((bookTitle.text.strip(), authorName.text.strip()))
...
def get_search_links():
    ...
    links = soup.find_all("a", class_="bookTitle")[0:10]
...
def get_book_summary(book_url):
    ...
    #authorstrip = author.text.strip()

    num_pages = soup.find('span', itemprop = 'numberOfPages').text.strip()
    regex = r"\d+"
    page_number = re.findall(regex, num_pages)

    #print(page_number)

    return (title, author, int(page_number[0])) 
...
def summarize_best_books(filepath):
    ...
    for book in books:
        ...
        img_tag = book.find('img', class_ = 'category__winnerImage')
        title = img_tag['alt']
...
class TestCases(unittest.TestCase):
    ...
    def test_get_titles_from_search_results(self):
        ...
        self.assertTrue(titles[0][0], 'Harry Potter and the Deathly Hallows (Harry Potter, #7)')
        self.assertTrue(titles[0][1], 'J.K. Rowling')

        # check that the last title is correct (open search_results.htm and find it)
        self.assertTrue(titles[-1][0], 'Harry Potter: The Prequel (Harry Potter, #0.5)')
        self.assertTrue(titles[-1][1], 'J.K. Rowling')
    ...
    def test_get_search_links(self):
        ...
        for i in urls:
            self.assertIn("https://www.goodreads.com/book/show/", i)
    ...
    def test_get_book_summary(self):
        ...
        for i in summaries:
            ...
            self.assertTrue(isinstance(i[0], str))
            self.assertTrue(isinstance(i[1], str)) 
            # check that the third element in the tuple, i.e. pages is an int
            self.assertTrue(isinstance(i[2], int))
    ...
    def test_write_csv(self):
        ...
        searches = get_titles_from_search_results("search_results.htm")

        # call write csv on the variable you saved and 'test.csv'
        write_csv(searches, "test.csv")
